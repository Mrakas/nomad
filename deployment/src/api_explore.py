...
class VisualNavigationAPI:
    ...
    def navigate(self, 
        ...
        if single_img is not None:
            if context_queue.len() < 4:
                context_queue.append(single_img)
            else:
                context_queue = context_queue[1:] + [single_img]
        else:
            context_queue = [PILImage.open(img).convert("RGB") for img in context_images] # 4imgs !!
